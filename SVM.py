...
path_metadatos = "Datos_procesados"
save_path = ""
...
for filename in os.listdir(path_features):
    if filename.endswith("_windows_features.npz"):
        ...
        y_aux = metadatos["tag"]
        X_class1 = X_aux[y_aux==1]
        X_class0 = X_aux[y_aux==0]
        y_class1 = y_aux[y_aux==1]
        y_class0 = y_aux[y_aux==0]
        X_aux = np.concatenate((X_class0, X_class1))
        y_aux = np.concatenate((y_class0, y_class1))
...
df_trials.to_parquet(os.path.join(save_path, "best_parameters.parquet"))